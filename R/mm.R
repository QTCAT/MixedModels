...
relMM <-  function(formula, data, family = NULL, REML = TRUE, 
  ...
  Zt <- lmf@pp$Zt
  for (i in seq_along(covarrel)) { # i <- 1
    tn <- which(match(pnms[i], names(lmf@flist)) == asgn) # tn <- 1
    if (length(tn) > 1)
      stop("a relatinships matrix must be associated with only one r.e. term")
    ind <- (lmf@Gp)[tn:(tn + 1L)]
    rowsi <- (ind[1] + 1L):ind[2]
    stopifnot(setequal(rownames(covarrel[[i]]), rownames(Zt)[rowsi]))
    relfac[[i]] <- tryCatch(chol(covarrel[[i]]),
                            error = function(x) {
                              message("a relatinships matrix is not positive definite")
                              message(x)
                              return(NULL)
                            })
